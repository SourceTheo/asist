...
# All Rights Reserved for "Theo - UserBot" "مرسليو "
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Theo - UserBot" " مرسليو "
    zed_dev = (6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993, 6367618993)
    malath = bot.uid
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝙏𝙝𝙚𝙤\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝙏𝙝𝙚𝙤\
