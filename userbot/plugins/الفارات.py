# Theo - UserBot
# Copyright (C) 2022 Theo . All Rights Reserved
#
# This file is a part of < https://github.com/SourceTheo/asist/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/SourceTheo/asist/blob/master/LICENSE/>.
...
حقـوق للتـاريخ : @Source_Theo  
@XIX_A - كتـابـة الملـف :  مرسليو """
#مرسليو يولـد هههههههههههههههههههههههههه
...
ZelzalVP_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗵𝗲𝗼  𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/Source_Theo  ) 𓆪\n\n"
    ...
    "\n𓆩 [𐇮 𝐌𝐀𝐑𝐂𝐄𝐋𝐎 𐇮](t.me/XIX_A  ) 𓆪"
)


# Copyright (C) 2022 Source_Theo   . All Rights Reserved
@Source_Theo  .on(zelzal_cmd(pattern=r"اضف (.*)"))
@Source_Theo  .on(sudo_cmd(pattern=r"اضف (.*)", allow_sudo=True))
async def variable(event):
    ...
    # All Rights Reserved for "Source_Theo   - UserBot" "زلـزال الهيبـه"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        variable = "ZED_MEDIA"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر {} بنجـاح ☑️**\n**✾╎المتغير : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت ثـيـو   يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغيـر المنطقـة الزمنيـه**\n **✾╎المتغير : دولـة مصـر 🇪🇬**\n\n**✾╎يتم الان اعـادة تشغيـل بـوت ثـيـو   يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        else:
...
# Copyright (C) 2022 Source_Theo   . All Rights Reserved
@Source_Theo  .on(zelzal_cmd(pattern="حذف(?:\s|$)([\s\S]*)"))
@Source_Theo  .on(sudo_cmd(pattern="حذف(?:\s|$)([\s\S]*)", allow_sudo=True))
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**✾╎تم حـذف {} بنجـاح ☑️**\n**✾╎المتغيـر المحـذوف : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت ثـيـو   يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
# Copyright (C) 2022 Source_Theo   . All Rights Reserved
@Source_Theo  .on(zelzal_cmd(pattern="جلب(?:\s|$)([\s\S]*)"))
@Source_Theo  .on(sudo_cmd(pattern="جلب(?:\s|$)([\s\S]*)", allow_sudo=True))
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
    ...
    if input_str == "رمز الوقتي" or input_str == "رمز الاسم الوقتي":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس : @Source_Theo  **".format(input_str, heroku_var[variable]))
        else:
...
# Copyright (C) 2022 Source_Theo   . All Rights Reserved
@Source_Theo  .on(zelzal_cmd(pattern="وقت(?:\s|$)([\s\S]*)"))
@Source_Theo  .on(sudo_cmd(pattern="وقت(?:\s|$)([\s\S]*)", allow_sudo=True))
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
    ...
    if input_str == "العراق" or input_str == "عراق":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المتغير : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت ثـيـو   يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
...
# Copyright (C) 2022 Source_Theo   . All Rights Reserved
@Source_Theo  .on(admin_cmd(pattern="رفع مطور$", outgoing=True))
@Source_Theo  .on(sudo_cmd(pattern="رفع مطور$", allow_sudo=True))
...
# Copyright (C) 2022 Source_Theo   . All Rights Reserved
@Source_Theo  .on(admin_cmd(pattern="تنزيل مطور$", outgoing=True))
@Source_Theo  .on(sudo_cmd(pattern="تنزيل مطور$", allow_sudo=True))
async def zelzalll(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
...
@Source_Theo  .on(sudo_cmd(pattern="اضف صورة (الحماية|الحمايه|الفحص|الوقتي) ?(.*)", allow_sudo=True))
async def _(tosh):
    if tosh.fwd_from:
        return
    if Config.HEROKU_API_KEY is None:
...
# Copyright (C) 2022 Source_Theo   . All Rights Reserved
@Source_Theo  .on(zelzal_cmd(pattern="اوامر الفارات"))
@Source_Theo  .on(sudo_cmd(pattern="اوامر الفارات", allow_sudo=True))
