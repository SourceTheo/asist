...
Heroku_cmd = (
    "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙏𝙝𝙚𝙤 - هيـروكو الفـارات](t.me/Source_Theo ) 𓆪\n"
    "**⌔∮ قائـمه اوامر هيروكو :** \n"
    "⪼ `.ضع فار` + الفار + المتغير\n"
    "⪼ `.جلب فار` + الفار لعرض ما في المتغير \n"
    "⪼ `.حذف فار` + الفار لحذف الفار \n"
    "⪼ `.استخدامي` \n"
    "\n𓆩 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙏𝙝𝙚𝙤](t.me/Source_Theo ) 𓆪"
)
...
async def variable(var):
    ...
    if exe == "جلب":
        ...
        if variable in heroku_var:
            await ics.edit("**⌔∮ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت ثـيـو  يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
...
async def _(dyno):
    ...
    try:
        ...
        return await dyno.reply(
            "**- عذرا لا يمكنك استخدام اوامر الفارات وهيروكو الا بعد اضافة كود هيروكو الى الفارات شرح الاضافة [اضغط هنا](https://t.me/Source_Theo  )**"
        )
    data = app.get_log()
    await edit_or_reply(
